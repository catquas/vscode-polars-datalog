 ...
 */
function dedupWrap(varName, innerExpr) {
    return (`{(lambda _i=id(${varName}),` +
...
function buildLogMessage(assignment, exportConfig) {
    ...
    if (hasCsv) {
        ...
        const logAction = logPath
            ...
            : '';
        const innerExpr = `(lambda _d, _r: (_d.mkdir(parents=True, exist_ok=True), ` +
        ...
        const logPath = exportConfig.logFileAbsPath.replace(/\\/g, '/');
        const v = assignment.varName;
        const innerExpr = `open('${logPath}', 'a').write(` +
            `__import__('datetime').datetime.now().strftime('[%H:%M:%S] ') + ` +
