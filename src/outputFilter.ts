 ...
 */
export class TracebackFilter {
  private readonly wsRoot: string; // forward-slash normalised
  private lineBuffer = '';
  private inTraceback = false;
  private tracebackLines: string[] = [];
  ...
  private emitTraceback(): string[] {
    const lines = this.tracebackLines;
    if (lines.length === 0) { return []; }

    const result: string[] = [lines[0]]; // "Traceback (most recent call last):"
    let keptFrames = 0;
    let i = 1;
    ...
    while (i < lines.length) {
      ...
      if (m) {
        ...
        const inWorkspace =
          filePath.startsWith(this.wsRoot + '/') || filePath === this.wsRoot;
