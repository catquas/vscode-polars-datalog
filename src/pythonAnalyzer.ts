export interface DataFrameAssignment {
  varName: string;
  sourceText: string;      // full multi-line source text of the assignment
  range: { startLine: number; endLine: number };  // 0-based line numbers
  inputVars: string[];     // known DataFrame vars referenced in RHS
}
 ...
 */
export function findInputVars(rhs: string, knownDfVars: Set<string>): string[] {
  const found: string[] = [];
